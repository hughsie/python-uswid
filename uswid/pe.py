...
class PeDosHeader:
    ...
    def __init__(self, buf: Optional[bytes] = None, offset: int = 0):
        ...
        self.e_cp: int = 0x0
        self.e_crlc: int = 0x0
        self.e_cparhdr: int = 0x0
        self.e_minalloc: int = 0x0
        self.e_maxalloc: int = 0x0
        self.e_ss: int = 0x0
        self.e_sp: int = 0x0
        self.e_csum: int = 0x0
        self.e_ip: int = 0x0
        self.e_cs: int = 0x0
        self.e_lfarlc: int = 0x0
        self.e_ovno: int = 0x0
        self.e_res: int = 0x0
        self.e_oemid: int = 0x0
        self.e_oeminfo: int = 0x0
        self.e_res2: int = 0x0
        self.e_lfanew: int = 0xB0
        if buf:
...
class PeDosStub:
    ...
    def __init__(self, buf: Optional[bytes] = None, offset: int = 0):
        self.RawData: Optional[bytes] = None
        if buf:
            self._import(buf, offset)
...
class PeCoffHeader:
    ...
    def __init__(self, buf: Optional[bytes] = None, offset: int = 0):
        ...
        self.SectionAlignment: int = 0x20
        self.FileAlignment: int = 0x20
...
class PeFile:
    def __init__(self, buf: Optional[bytes] = None):
        ...
        self.data_directories: List[PeDataDirectory] = []
...
def main():
    ...
    with open(fn + ".txt", "wb") as f:
        pe = pefile.PE(fn)
        f.write(str(pe).encode())

    # open it up ourselves
    ...
    # dump our state
    with open(fn + ".export", "wb") as f:
        f.write(str(mype).encode())

    # log what we created
    with open(fn + ".new.txt", "wb") as f:
        pe = pefile.PE(fn + ".new")
        f.write(str(pe).encode())
